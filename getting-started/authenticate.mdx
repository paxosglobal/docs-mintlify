...
The [Market Data](/api/v2#tag/Market-Data) and most [Pricing](/api/v2#tag/Pricing) endpoints do not require authorization.
</Tip>

For example, the following scopes (space delimited) should provide sufficient permissions to mint, redeem, and convert Paxos-issued stablecoins:
...
Once you have the `access_token`, [make an API call in Sandbox](request).
