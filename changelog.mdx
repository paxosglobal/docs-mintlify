...
rss: true
---
<Update label="March 9, 2026" description="v2 API">
**Identity API**: Renamed `summary_status` to `status` on the Identity object. Renamed `old_summary_status` and `new_summary_status` to `old_status` and `new_status` on [identity status change webhook events](/api-reference/events/identity-summary-status-change-object).
</Update>
