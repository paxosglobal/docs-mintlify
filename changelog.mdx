...
**Identity Controls API**: Added new [Identity Controls API](/api-reference/endpoints/identity-controls/overview) for managing identity lifecycle states. This API replaces the deprecated `user_disabled` and `admin_disabled` fields on Identity and provides finer-grained control types including `SELL_ONLY`, `CLOSED`, `FROZEN`, and `DORMANT`. See the [Dormancy Lifecycle guide](/guides/identity/dormancy) for managing inactive identities.
</Update>
