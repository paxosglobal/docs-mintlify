...
rss: true
---
<Update label="April 24, 2026" description="Deprecation">
**Stablecoin Conversions API Deprecated**: As [announced in January](#january-22-2026), the Stablecoin Conversions API is now deprecated. New customers should use the [Orchestrations API](/api-reference/endpoints/orchestration/overview). Existing customers should follow the [migration guide](/api-reference/endpoints/stablecoin-conversion/overview#migration-guide).
</Update>
...
**Identity Controls API**: Added new [Identity Controls API](/api-reference/endpoints/identity-controls/overview) for managing identity access. Control types include `SELL_ONLY`, `CLOSED`, `FROZEN`, and `DORMANT`. See the [Dormancy Lifecycle guide](/guides/identity/dormancy) for managing inactive identities.
</Update>

<Update label="March 20, 2026" description="v2 API">
**Transfer Status Details**: Add `stale_fee_quote` to `status_details` on Transfer responses. This field is populated when a crypto withdrawal is cancelled because on-chain state changed after the fee was quoted, indicating that the fee quote must be refreshed before retrying.
</Update>

<Update label="March 3, 2026" description="v2 API">
**Profile Transaction Reconciliation Reports**: Paxos is enhancing reconciliation reports with the following changes: Orchestration API transactions will have additional entries in the report: New Resource Type: `ORCHESTRATION`; New Resource Transaction Types: `ORCHESTRATION_RESERVE`, which reserves funds during an orchestration flow, and `ORCHESTRATION_UNRESERVE`, which returns funds in case of an orchestration failure. These changes will improve tracking of multi-step transactions and provide better visibility into fund allocation during processing.
</Update>
...
<Update label="February 5, 2026" description="v2 API">
**v2 API**: Add `RTP_DEPOSIT` transfer type and `RTP` fiat network. Add `RTP` to Fiat Network Instructions.
</Update>

<Update label="February 4, 2026" description="v2 API, Deprecation">
**Orchestration API response field deprecation**: The `rules` and `orchestrations` fields in the [List Orchestration Rules](/api-reference/endpoints/orchestration-rules/list-orchestration-rules) and [List Orchestrations](/api-reference/endpoints/orchestration/list-orchestrations) endpoint responses are deprecated. A new standardized `items` field replaces them. Both fields are returned during the transition period. The deprecated fields will be removed 90 days after this announcement (May 2026).
</Update>

<Update label="February 4, 2026" description="Stablecoin">
**USDG Contract Update**: Paxos will implement contract updates for USDG across all supported EVM networks to enhance system security. No service interruptions or processing delays are expected.
</Update>

<Update label="January 22, 2026" description="Deprecation">
**Stablecoin Conversions API Deprecation**: The Stablecoin Conversions API service will be deprecated on April 22, 2026. Customers using the Stablecoin Conversions API should migrate to the [Orchestrations API](/guides/developer/orchestrations/orchestrations) for conversions.
</Update>
...
<Update label="December 1, 2025" description="Dashboard">
    **Configurable Approvers**: You can now edit the number of approvals required for a given policy from the Dashboard. Navigate to **Admin > Approvals** to modify existing approval policy configurations. See the [Dashboard Approvals guide](/guides/dashboard/admin/approvals) for more information.
</Update>

<Update label="November 12, 2025" description="Webhooks, Docs">
    **Self-Service Webhooks**: Self-serve webhooks are now live. You can create and manage webhooks yourself via your Dashboard account and send test events via the Dashboard to verify functionality. Please refer to the updated [Webhooks guide](/guides/webhooks) for more information.
</Update>

<Update label="October 31, 2025" description="v2 API">
    **List & Get Events API**: Added new `is_test` filter parameter to List Events and Get Event endpoints. Use this filter to query for real or test events. The default is `is_test=false`, which queries for real events only.
</Update>

<Update label="October 29, 2025" description="Docs">
    **Webhooks & Events API**: Updated documentation around Webhooks and the Events API around the introduction of the `is_test` field in the Event object. The `is_test` field indicates whether the event is a test event or a production event.
</Update>

<Update label="Oct 27, 2025" description="v2 API">
    **V2 API**: Updated TRUST institution member requirements. Trust institutions now require only one member with one of the following roles: GRANTOR, TRUSTEE, or BENEFICIARY. Previously, trust institutions required members that collectively had all three roles (BENEFICIARY, TRUSTEE, and GRANTOR). BENEFICIAL_OWNER is no longer applicable for trust institutions.
</Update>
...
**Docs, v2 API**: Add new [Settlement APIs](https://docs.paxos.com/api/v2#tag/Settlement) which allow for atomic one-way or two-way transfers across Paxos accounts. {/* https://itbitwiki.atlassian.net/browse/LGR-1925 */}
