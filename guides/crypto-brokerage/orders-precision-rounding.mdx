...
- `quote_amount` tick size of 0.01 (1e-2) for [Create Order](/endpoints/orders/create-order).
- `volume_weighted_average_price` tick size of 0.00000001 (1e-8) for [Get Order](/endpoints/orders/get-order) and [List Orders](/endpoints/orders/list-orders).
- `average_price` precision of 0.000001 (1e-6) for [List Historical Prices](/endpoints/pricing/list-historical-prices).
- `commission ` and `rebate` tick size of 0.0000000000000000001 (1e-19) for [List Executions](/endpoints/orders/list-executions).
- `gross_trade_amount` tick size of 0.00000000000001 (1e-14) for [List Executions](/endpoints/orders/list-executions).
- `amount` precision of 0.00000001 (1e-8) for [Get Ticker](/endpoints/market-data/get-ticker), [Get Order Book](/endpoints/market-data/get-order-book) and [List Executions](/endpoints/orders/list-executions).
- `base_amount` precision of 0.00000001 (1e-8) for [List Orders](/endpoints/orders/list-orders), [Create Order](/endpoints/orders/create-order) and [Get Order](/endpoints/orders/get-order).
- `amount_filled` precision of 0.00000001 (1e-8) for [List Orders](/endpoints/orders/list-orders), [Create Order](/endpoints/orders/create-order) and [Get Order](/endpoints/orders/get-order).
...
This provides the volume-weighted average price across all underlying executions for an order within [List Executions](/endpoints/orders/list-executions).
