...
Use **OrderRoutingMode (`8000`)** to choose the routing strategy for a new order.
...
To apply **OrderRoutingMode (`8000`)**, the user/customer must be whitelisted by Paxos engineers. If the account is not whitelisted, this field is ignored.

</Note>

<Note>

If an invalid value is provided for **OrderRoutingMode (`8000`)**, the field is ignored and the routing mode configured for the user/customer is used.
