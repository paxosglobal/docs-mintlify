...
Log in to your [Dashboard](https://dashboard.paxos.com/login) account and navigate to the Webhooks section under the Developer tab. Configure your webhook with:
 ...
 - **Authentication**: Configure your selected authentication method
   ...
   - **OAuth**: Provide the OAuth endpoint URL (client_url), Client ID, and Client Secret. When using OAuth2 authentication, access tokens are obtained by sending a POST request to the configured client_url. The credentials are sent in the request body using `application/x-www-form-urlencoded` format:
     ```
