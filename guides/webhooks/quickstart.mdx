...
When using OAuth2 authentication, access tokens are obtained by sending a POST request to the configured `client_url`. The credentials are sent in the request body using `application/x-www-form-urlencoded` format:
