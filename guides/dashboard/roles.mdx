...
All Organizations have at least one Organization Administrator with permission to onboard Entities. Organization Administrators who complete the onboarding for an Entity automatically become the [Entity Manager](#entity-manager) for that Entity.

## Entity Manager

Entity Managers assign roles, add users, map identity provider groups to roles, etc. Entity Managers can [review Entity specific settings](https://dashboard.paxos.com/admin). Entity Managers can be reassigned once an Entity has been created. Entity Managers have view permissions across the Dashboard, but cannot perform treasury focused actions.
...
Has read-only access across dashboard features with additional permissions for onboarding.
...
Custom roles can be created from the **Team Management > Roles** tab via the `Add Custom Role` button.

![Add Custom Role Button](/images/add-custom-role-button.png)

This will allow you to name your role and toggle the relevant permissions you desire this role to have. Role names must be unique. You can also optionally set a description for the custom role.

![Custom Role Permissions](/images/custom-role-permissions.png)

<Note>
  Some permissions will be flagged as `Requires KYC`. Permissions that enable the movement of funds (either on/off platform or between entities) require users to be [KYC verified](/guides/dashboard/verified-users-for-sensitive-permissions) before being able to use them.
</Note>