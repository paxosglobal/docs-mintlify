...
Passkey authentication requires your device's system time to be accurate. If your system time is more than 30 seconds off from the actual current time, authentication may fail.

<Tip>

 Keep Your System Time Synchronized

To ensure reliable passkey authentication, use an NTP (Network Time Protocol) service to automatically keep your system time synchronized with an NTP server. Most operating systems include NTP configuration options. Contact your IT organization if you need assistance setting up time synchronization.
