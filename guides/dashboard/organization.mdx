...
Within the Organization, individual access is selectively provisioned across Entities, so an individual can only access the information and funds and based on their [roles](/dashboard/roles) within the respective Entity.
...
When you [sign up for the Paxos Dashboard](/dashboard/account), you are creating [Organization Administrator](/dashboard/roles#organization-administrator) credentials to log into a Dashboard Organization.
...
- Roles & Permissions, APIs keys are scoped at an Entity level. For example, if an Organization has 3 entities, User A has a Treasurer role in Entity 1, a Viewer role in Entity 2, and no access to Entity 3. See [Roles and Responsibilities](/dashboard/roles).
- [Organization Administrators](/dashboard/roles#organization-administrator) who complete the onboarding for an Entity automatically become the [Entity Manager](/dashboard/roles#entity-manager) for that new Entity. Entity Managers are allowed to assign roles, add users, map identity provider groups to roles, etc. Entity Managers can be reassigned later
...
For example, [depositing USD](/dashboard/deposit) into a Profile is the first step when [minting](/dashboard/mint) a stablecoin.
