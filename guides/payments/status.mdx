...
- [List Transfers](/api-reference/endpoints/transfers/list-transfers): When [monitoring a pay-in](/guides/payments/quickstart#pay-in-monitor-reconcile), a `PENDING` a transaction is being confirmed. Depending on the network, confirmation can take from seconds to minutes. Transactions with a `COMPLETED` status have received sufficient confirmations. The `FAILED` status means the transaction was denied.
