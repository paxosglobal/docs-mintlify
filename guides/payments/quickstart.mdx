...
However, any existing Profile can be used when setting up simple [pay-in](/payments/quickstart#payment-workflow) and [refund](/payments/quickstart#refunds) workflows.
...
To process stablecoin refunds on the Paxos Platform, first [fund your account with USD](/payments/quickstart#refund-fund-account) then use  [Create Crypto Withdrawal](/api-reference/endpoints/crypto-withdrawals/create-crypto-withdrawal) to initiate a stablecoin refund to the Buyer.
