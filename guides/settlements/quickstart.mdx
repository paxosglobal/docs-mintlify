...
The [settlement workflow](/settlements#workflow) requires [two Profiles](#set-up-profiles), referred to in this guide as Source Profile (`source_profile_id`) and Target Profile (`target_profile_id`).
...
Either party can use [Get Transaction](/api-reference/endpoints/settlement/get-transaction) to check the [status](status).
