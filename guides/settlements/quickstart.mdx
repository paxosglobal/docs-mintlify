...
> Contact your [Entity Manager](/dashboard/roles#entity-manager) or [Support](https://support.paxos.com) if you run into any issues or don't have access to a Sandbox Account.
...
The transaction [status](status) can be used used by either party to [retrieve a list of filtered transactions](status#list-transactions).
...
The acknowledgement response includes transaction information and the `AFFIRMED` [status](status) and `updated_at` timestamp.
