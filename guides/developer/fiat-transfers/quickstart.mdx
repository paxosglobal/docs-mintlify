...
This guides walks you through the steps to [send fiat to the Paxos Platform](#funding) as well as [move money to an external bank account](#withdrawal) using the [Fiat Transfers](/api-reference/fiat-transfers), [Transfers](/api-reference/transfers) and [Profiles](/api-reference/profiles) APIs.
