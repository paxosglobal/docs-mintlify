...
## Prerequisites for External Crypto Destinations

When sending funds to external crypto addresses in orchestrations, you can optionally pre-register the destination address using [Put Crypto Destination Address](/api-reference/endpoints/crypto-destination-addresses/put-crypto-destination-address). This allows you to:

- Store travel rule metadata for compliance
- Reuse addresses across multiple orchestrations using the `crypto_address_id`
...
### Using Pre-registered Addresses in Orchestrations

Once registered, you can reference the address by its ID in orchestration destinations:
...
Alternatively, you can specify the address directly without pre-registration:
