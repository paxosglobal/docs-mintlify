...
[deposit-address]: /api-reference/deposit-addressesCreateDepositAddress
[conversion-api]: /api-reference/stablecoin-conversion/create-stablecoin-conversion
[conversion guide]: developer/convert.mdx
[fiat-withdraw]: /api-reference/fiat-transfers/operation/CreateFiatWithdrawal
[fiat withdrawal guide]: developer/fiat-transfers/quickstart.mdx#withdrawal
[List Transfers]: /api-reference/transfers/list-transfers
[List Deposit Addresses]: /api-reference/deposit-addressesListDepositAddresses
[token]: developer/authenticate.mdx



![Redeem Workflow](/img/redeem.png)

To redeem a [Paxos-issued stablecoin][stablecoins], use [Create Deposit Address][deposit-address] to add the appropriate address on the Paxos platform. Next, initiate a stablecoin transfer from your external address (**A** in the diagram).
...
Then, use [Create Fiat Withdrawal][fiat-withdraw] to send USD to your bank account (**C**).

<Tip>

- To redeem stablecoins you already hold on the Paxos platform, you can use [Create Stablecoin Conversion][conversion-api] to easily convert to USD. See the [conversion guide] for details.
...
Use [Create Deposit Address][deposit-address] to add a wallet address to the designated Profile.
...
Initiate an on-chain transfer from your external wallet to the `address` included in the response from the [previous step](#add-deposit-address) or from [List Deposit Addresses].
...
Use [Create Fiat Withdrawal][fiat-withdraw] to send USD to your bank account.
You'll need to first save your external banking information on the Paxos platform.
See the [fiat withdrawal guide] for details.
