    ...
    __html: `
      ...
      function parseApiUrl() {
        ...
        if (!hash) {
          ...
          hash = urlParams.get('fragment') || urlParams.get('from') || '';
          console.log('Checking URL params for hash:', hash);
        }

        // Check if we're coming from an /api/v2 URL and extract from pathname
        if (!hash && window.location.pathname.includes('/api/v2/')) {
          // Extract the path after /api/v2/
          const pathParts = window.location.pathname.split('/api/v2/');
          if (pathParts.length > 1) {
            hash = pathParts[1];
            console.log('Extracted from pathname:', hash);
          }
        }
        ...
        try {
          ...
          setTimeout(() => {
            ...
            if (fallbackEl) {
              ...
              const linkEl = document.getElementById('fallback-link');
              if (linkEl) {
          ...
          // Perform the redirect immediately (reduce delay)
          setTimeout(() => {
          ...
          setTimeout(() => {
            console.log('Fallback redirect to introduction');
            window.location.href = '/api-reference/introduction';
          }, 1000);
        }
      }

      // Start the redirect process with multiple triggers
      ...
      window.addEventListener('load', performRedirect);
      
      // Try after a small delay to ensure everything is initialized
