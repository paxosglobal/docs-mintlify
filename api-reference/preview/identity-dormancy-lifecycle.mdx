...
Once an identity is no longer dormant, the identity will typically return to the `APPROVED` status after re-screening if no other controls or requirements apply.
You can listen for an [identity status change event](/api-reference/events/identity-status-change-object#identity-summary-status-change) via [Paxos Webhooks](/guides/webhooks) to be notified when the identity's status changes.
