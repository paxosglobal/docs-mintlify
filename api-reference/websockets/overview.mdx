...
[execution-data]: #execution-data-feed
...
[list-recent-executions]: /api-reference/endpoints/market-data/list-recent-executions
...
Paxos provides secure, real-time market and execution data feeds via public [WebSocket][websocket] connections. The WebSocket API offers three main data feeds:

- **[Execution Data Feed][execution-data]** - Real-time execution data for market trades
- **[Market Data Feed][market-data]** - Order book updates with initial snapshots and real-time updates  
- **[Stablecoin Market Price Feed][stablecoin-data]** - Market index price updates for external stablecoins
...
Market availability varies by environment (Sandbox vs Production) and account restrictions. Use the [Get Order Book][get-order-book] and [List Recent Executions][list-recent-executions] REST endpoints to discover available markets, though these are not suitable for streaming data.
